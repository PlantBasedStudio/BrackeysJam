...
public class PlayerTakeLight : MonoBehaviour
{
    ...
    public void OnUse()
    {
        ...
        {
            if (hit.collider.TryGetComponent<Flash>(out Flash flash))
            {
    ...
    private void Update()
    {
        if (Physics.Raycast(Camera.position, Camera.forward, out RaycastHit hit, MaxUseDistance, UseLayers) && (hit.collider.name.Equals("Flashlamp")))
        {
            Debug.Log("Press E");
